...
import 'package:flutter_sado_ecommerce/data/model/response/transaction_model.dart';
...
import 'package:flutter_sado_ecommerce/view/screen/payment/payment_screen.dart';
import 'package:flutter_sado_ecommerce/view/screen/wallet/model/wallet_bonus_model.dart';
...
class WalletTransactionProvider extends ChangeNotifier {
  ...
  Future<void> getLoyaltyPointList(BuildContext context, int offset,
    ...
    _isLoading = true;
    ApiResponse apiResponse =
        await transactionRepo!.getLoyaltyPointList(offset);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
  ...
  Future convertPointToCurrency(BuildContext context, int point) async {
    ...
    if (apiResponse.response != null &&
      ...
      ScaffoldMessenger.of(Get.context!).showSnackBar(SnackBar(
        backgroundColor: Colors.green,
        content:
            Text('${getTranslated('point_converted_failed', Get.context!)}'),
      ));
    }
    notifyListeners();
  }
  ...
  Future<void> addFundToWallet(String amount, String paymentMethod) async {
    ...
    if (apiResponse.response != null &&
      ...
      print(" vai == ${apiResponse.response?.statusCode}");
      _isConvert = false;
      String? errorMessage;
